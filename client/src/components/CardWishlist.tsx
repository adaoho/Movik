...
const CardWishlist = ({ product }: { product: ProductsPageType }) => {
  return (
    <>
      <div className="flex flex-col bg-white w-[100%] h-[420px] rounded-sm shadow-lg relative overflow-hidden hover:scale-105 transition-all">
        <div className="flex h-[80%] w-full overflow-hidden items-center justify-center p-[5%] -mb-[5%]">
          {/* Picture of The Card */}
          <img
        ...
        <div className="flex flex-col w-full px-[7%] pt-4 gap-y-1">
          ...
          <p className="text-[11px] text-gray-500">{product.excerpt}</p>
        </div>
        {/* Price Tag */}
        ...
        <Link
          href={`/wishlist`}
          className="flex flex-row w-full justify-center items-center h-[11%] pb-2 mt-2 z-10"
        >
          {/* Button Add Wishlist */}
          <div className="flex w-[92%] h-full bg-gray-900 items-center justify-center rounded-md flex-row gap-x-1 hover:bg-orange-900 transition-all">
            ...
            <p className="text-[13px]">Remove Wishlist</p>
          </div>
        </Link>
      </div>
    </>
